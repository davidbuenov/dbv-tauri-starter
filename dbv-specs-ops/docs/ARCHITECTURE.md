...
- **Restricción:** Este repo **nunca empuja tags con el patrón `vX.Y.Z`** — los 3 `release-{windows,linux,macos}.yml` se disparan con ese mismo patrón (`push: tags: ["v*.*.*"]`), y ese tag debe quedar reservado para cuando exista un proyecto derivado real con una app que sí tenga sentido publicar. La evolución del propio starter se versiona con tags `template-vX.Y.Z` (prefijo distinto, no casa con el trigger) y su propio `CHANGELOG.md` en la raíz — separado de `dbv-specs-ops/CHANGELOG.md`, que es el changelog del framework, no del starter.
