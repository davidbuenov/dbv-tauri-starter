...
- **Foco inmediato:** Framework en v2.7.0 tras la cosecha de lecciones de las 3 apps ya publicadas. Siguiente hito: planificación de la migración de `dbv-pdf2deck` (arquetipo D, sidecar Python), bloqueada por la auditoría de licencia de PyMuPDF.
...
- **2026-08-23 - Validación de extremo a extremo con proyecto derivado real: DBV Teleprompter.** Quedaba pendiente (ver entrada anterior) confirmar que el patrón de CI (`release-*.yml`) y el propio starter servían para un proyecto real, no solo para la demo. `DBV Teleprompter` se completó, se convirtió a aplicación de escritorio con este starter y se generaron los ejecutables/instaladores para tienda. Ahora mismo en proceso de revisión en **Microsoft Store** y **Uptodown**. Cierra la pregunta abierta de `docs/SPECIFICATIONS.md` §7 — el starter queda validado como plantilla reutilizable, no solo como demo interna.

- **2026-08-28 - Cosecha de lecciones al framework (v2.7.0) tras 3 apps en tienda.** Con `dbv-md-reader`, `dbv-teleprompter` y `eer-studio` ya publicadas, se auditaron los `memory.md` de los tres proyectos contra los 4 documentos de escritorio del framework para localizar lecciones que nunca se generalizaron. **Hallazgo principal:** la "DoD de Experiencia de Escritorio" que el ADR de `dbv-teleprompter` (2026-08-22) daba por formalizada en `NATIVE_DESKTOP_APPS.md §7` **no existía en el framework** — ese §7 nunca se escribió. Es el segundo caso idéntico en una semana (el primero, el menú nativo de macOS, detectado en la sesión de `eer-studio`). **Lección de proceso, no de Tauri:** un ADR que *declara* haber backporteado algo no es prueba de que el backport ocurriera; el Memory Gate de `/ship` debe verificar la sección en el fichero real del framework, no fiarse de la frase del ADR. Además se recogieron 4 trampas nuevas de Tauri v2, la regla de que la clave del updater la genera el usuario y no la IA, el trackeo íntegro de `gen/windows/`, y dos gotchas de CI. Detalle completo en `dbv-specs-ops/CHANGELOG.md` §2.7.0.
